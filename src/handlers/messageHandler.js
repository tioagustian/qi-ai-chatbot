...
import { generateAIResponseLegacy, analyzeImage, storeImageAnalysis, generateImage, generateAnalysis } from '../services/aiService.js';
...
async function processMessage(sock, message) {
  try {
    ...
    if (commandData) {
      ...
      // Special handling for !status command to include mood and personality change information
      if (commandData.command === 'status') {
        // Get more detailed mood and personality info
        ...
        executeCommand = async (...args) => {
          ...
          if (response && commandData.command === 'status') {
      ...
      try {
        ...
        if (response) {
          ...
          const responseDelay = calculateResponseDelay(content, response);
        ...
        await sock.sendMessage(chatId, { text: `Error executing command: ${commandError.message}` });
      }
    ...
    try {
      await updateContext(db, chatId, sender, senderName, content || '', sock);
    } catch (contextError) {
      logger.error('Error updating context', contextError);
    }
    
    // Determine if we should respond to this message
    const shouldRespondToThisMessage = shouldRespondToMessage(message, content, isTagged, isGroup, db.data.config.botName);
    
    // Check for image generation request
    const generationRequest = detectImageGenerationRequest(content);
    
    if (shouldRespondToThisMessage || generationRequest) {
      // Get relevant context
      let relevantContext = await getRelevantContext(db, chatId, content, isGroup);
      
      // Try to update mood/personality based on basic keyword matching
      await updateMoodAndPersonality(db, content);
      
      // Add more sophisticated AI-based mood and personality analysis
      // Only do this for certain messages to avoid excessive API calls
      const shouldAnalyzeMood = isTagged || // The bot is explicitly mentioned
        !isGroup || // It's a private chat
        Math.random() < 0.25; // Random chance for group messages (25%)
        
      if (shouldAnalyzeMood && content && content.length > 5) {
        try {
          logger.info('Performing AI-based mood/personality analysis');
          
          // Get more context for AI analysis
          await updateMoodAndPersonalityWithAI(db, content, relevantContext, { generateAnalysis });
          
          // Update context with relevant facts after mood analysis
          relevantContext = await getRelevantContext(db, chatId, content, isGroup);
        } catch (moodAnalysisError) {
          logger.error('Error in AI-based mood analysis:', moodAnalysisError);
        }
      }
      ...
      if (isExplicitImageAnalysisRequest) {
        shouldRespondToThisMessage = true;
      }
      ...
      if (shouldRespondToThisMessage) {
        try {
          ...
          if (relevantContext.length > 0) {
            logger.info(`Adding ${relevantContext.length} relevant facts to context`);
            
            const factsString = relevantContext.join(', ');
            contextMessages.push({
              role: 'system',
