...
async function requestGeminiChat(model, apiKey, messages, params) {
  ...
  try {
    ...
    // Try NVIDIA API as fallback
    try {
      logger.info('Attempting to fallback to NVIDIA API');
      ...
      // Convert messages to NVIDIA API format
      ...
      // Prepare NVIDIA request payload
      const nvidiaRequestData = {
        messages: nvidiaMessages,
        model: 'meta/llama-3.3-70b-instruct', // Use Llama 3.3 as default fallback
        temperature: params.temperature || 0.7,
      ...
      logger.debug('NVIDIA fallback request:', {
      ...
      logger.success('Successfully got response from NVIDIA API fallback');
      
      // Extract content from NVIDIA response
      if (nvidiaResponse.data && 
          nvidiaResponse.data.choices && 
          nvidiaResponse.data.choices.length > 0 &&
          nvidiaResponse.data.choices[0].message) {
            
        // Create formatted response matching our expected format
        const formattedNvidiaResponse = {
          choices: [{
            message: {
              role: 'assistant',
              content: nvidiaResponse.data.choices[0].message.content
            }
          }]
        };
        ...
        await logApiRequest(
          ...
          },
          {
            executionTime: Date.now() - startTime,
            messageCount: messages.length,
            promptTokens: nvidiaResponse.data.usage?.prompt_tokens || 0,
            completionTokens: nvidiaResponse.data.usage?.completion_tokens || 0,
            success: true,
            fallback: true,
        ...
        return formattedNvidiaResponse;
      } else {
        throw new Error('Invalid response format from NVIDIA API');
      }
    } catch (nvidiaError) {
      // If NVIDIA fallback fails, log and rethrow original error
      logger.error('NVIDIA API fallback also failed:', nvidiaError);
      ...
      await logApiRequest(
        ...
        nvidiaError.response?.data || { error: nvidiaError.message },
        {
          executionTime: Date.now() - startTime,
          messageCount: messages.length,
          success: false,
